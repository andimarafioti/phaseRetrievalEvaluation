%load('diff_data_midi_speech.mat')
Markers = {'d','+','s','>','<','^', 'o','*','x','v',};
...
for index = 1:length(red)
    plotDiffData(1, -means_SC_PGHI(:, :, index), M, tfrs(:, index), 'SC PGHI', [0, 60], colors(2*index-1, :))
end

for index = 1:length(red)
    plotDiffData(2, -means_SC_SPSI(:, :, index), M, tfrs(:, index), 'SC SPSI', [0, 60], colors(2*index-1, :))
end

for index = 1:length(red)
    plotDiffData(3, means_ODG_PGHI(:, :, index), M, tfrs(:, index), 'ODG PGHI', [-4, 0.5], colors(2*index-1, :))
end

for index = 1:length(red)
    plotDiffData(4, means_ODG_SPSI(:, :, index), M, tfrs(:, index), 'ODG SPSI', [-4, 0.5], colors(2*index-1, :))
end
