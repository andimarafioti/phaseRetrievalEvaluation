...
for index = 1:length(red)
    plotDiffData(1, -means_SC_PGHI(1:3, :, index), M, tfrs(:, index), 'SNR_{MS} PGHI', [0, 60], colors(2*index-1, :))
end

for index = 2
    plotDiffData(2, -means_SC_SPSI(1:3, :, index), M, tfrs(:, index), 'SNR_{MS} SPSI', [0, 60], colors(2*index-1, :))
end
...
for index = 1:length(red)
    plotDiffData(5, -means_SC_FGLA(:, 1:end-1, index), M, tfrs(1:end-1, index), 'SNR_{MS} FGLA', [0, 60], colors(2*(index+1)-1, :))
end

for index = 1:length(red)
    plotDiffData(6, means_ODG_FGLA(:, 1:end-1, index), M, tfrs(1:end-1, index), 'ODG FGLA', [-4, 0.5], colors(2*(index+1)-1, :))
end
